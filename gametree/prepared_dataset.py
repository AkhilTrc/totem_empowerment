...
class PreparedDataset():
    ...
    """
    def __init__(self, data, game_version='totem', prediction_model=0, exclusion_elements=None, manual_validation=None,
        ...
        Args:
            data (tuple): Can either be a single DataFrame (total data) or a tuple consisting of two DataFrames (separate train and test data).
                        DataFrames contain information about elements involved in combination (columns 'first', 'second', 'third, element indices)
    ...
    def split_train_val_test_sets(self, data): #TODOOOOOO
        ...
        Args:
            data (tuple): Can either be a single DataFrame (total data) or a tuple consisting of two DataFrames (separate train and test data).
                        DataFrames contain information about elements involved in combination (columns 'first', 'second', 'third and element indices)
                        and its success and resulting element (column 'success', is either 1 or 0 and column 'result', element index or NaN).
        ...
        if self.exclusion_elements is None:
            ...
            third_exclusion_test = table.eval('third in @self.exclusion_elements[0]')
            if self.prediction_model != 1:
                test_condition = (first_exclusion_test | second_exclusion_test | third_exclusion_test)
            else:
                result_exclusion_test = table.eval('result in @self.exclusion_elements[0]')
                test_condition = (first_exclusion_test | second_exclusion_test | third_exclusion_test | result_exclusion_test)
            ...
            if len(self.exclusion_elements) == 2:
                ...
                third_exclusion_val = table.eval('third in @self.exclusion_elements[1]')
                if self.prediction_model != 1:
                    val_condition = (first_exclusion_val | second_exclusion_val | third_exclusion_val)
                else:
                    result_exclusion_val = table.eval('result in @self.exclusion_elements[1]')
                    val_condition = (first_exclusion_val | second_exclusion_val | third_exclusion_val | result_exclusion_val)
    ...
    def analyze_imbalance(self, *tables):
        ...
        Args;
            tables (Dataframe): Contains information about elements involved in combination (columns 'first', 'second', 'third' and element indices)
                        and its success and resulting element (column 'success', is either 1 or 0 and column 'result', element index or NaN).
        ...
        class_zero, class_one = np.bincount(class_values)   # Number of succesful and failed combinations. 
    ...
    def apply_oversampling(self, table):
        ...
        Args:
            table (DataFrame): Contains information about elements involved in combination (columns 'first', 'second', 'third' and element indices)
                        and its success and resulting element (column 'success', is either 1 or 0 and column 'result', element index or NaN).
                        Classes might be imbalanced.

        Returns:
            (DataFrame): Contains information about elements involved in combination (columns 'first', 'second', 'third', element indices)
                        and its success and resulting element (column 'success', is either 1 or 0 and column 'result', element index or NaN).
        ...
        table = pd.DataFrame({'first': table[:, 0], 'second': table[:, 1], 'third': table[:, 2], 'success': table[:,2], 'result': table[:,3]})
    ...
    def get_feature_and_label_vector(self, data, type=0):
        ...
        Args:
            data (DataFrame): Contains information about elements involved in combination (columns 'first', 'second', 'third' and element indices)
                        and its success and resulting element (column 'success', is either 1 or 0 and column 'result', element index or NaN).
        ...
        feature_vectors = np.empty((2*combination_count,3*vector_dim))      # Create empty matrix with the 2*dimensions of above measures.
        ...
        indices = np.empty((2*combination_count,5))

        # create combination features and get corresponding result vectors
        for idx, combination in enumerate(data):
            combination_success = [int(x) for x in combination[:4]]  
            result = combination[4]
            feature_vectors[idx,:] = np.concatenate([element_vectors[combination_success[0]], element_vectors[combination_success[1]], element_vectors[combination_success[2]]], axis = 0)    # Contains word vectors for the first, second and third elements for the combination.
            feature_vectors[idx+combination_count,:] = np.concatenate([element_vectors[combination_success[2]], element_vectors[combination_success[1]], element_vectors[combination_success[0]]], axis = 0)    # # Contains word vectors for the first, second and third elements for the combination reversed.

            if self.prediction_model == 0:  # Means Link prediction. 
                label_vector[idx] = combination_success[3]      # 0/1 based on success/failure of combination.
                label_vector[idx+combination_count] = combination_success[3]    # 0/1 based on s/f of combination reversed. 
                ...
                if combination[0] in self.combination_table:
                    if combination[1] in self.combination_table[combination[0]]:
                        result_elements.update(self.combination_table[combination[0]][combination[1]])
                    if combination[2] in self.combination_table[combination[0]]:
                        result_elements.update(self.combination_table[combination[0]][combination[2]])
                if combination[1] in self.combination_table and combination[2] in self.combination_table[combination[1]]:
                    result_elements.update(self.combination_table[combination[1]][combination[2]])
                else:
