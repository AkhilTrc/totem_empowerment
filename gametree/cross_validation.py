...
import pandas as pd
from gametree.element_prediction_model import \
    ElementPredictionModel
from gametree.link_prediction_model import \
    LinkPredictionModel
from gametree.empowerment_prediction_model import \
    EmpowermentPredictionModel
from gametree.prepared_dataset import PreparedDataset
...
class CrossValidation():
    ...
    def run_cross_validation(self):
        ...
        if self.prediction_model == 0:
            results.to_csv('gametree/data/{}/{}LinkPred-metrics.csv'.format(self.time, self.game_version), index=False)
        elif self.prediction_model == 1:
            results.to_csv('gametree/data/{}/{}ElemPred-metrics.csv'.format(self.time, self.game_version), index=False)
        else:
            results.to_csv('gametree/data/{}/{}EmpPred-metrics.csv'.format(self.time, self.game_version), index=False)
